...
const directZscore = (rawScore, mean, standardDeviation) => {
  let zscore;
  zscore = (rawScore - mean) / standardDeviation;
  return zscore.toFixed(2);
};

const inverseZscore = (rawScore, mean, standardDeviation) => {
  let zscore;
  zscore = (mean - rawScore) / standardDeviation;
  return zscore.toFixed(2);
};

const calculatePercentil = (zscore) => {
  if (zscore > 3 || zscore < -3) {
    let data = zscore > 0 ? 100 : 0;
    return { zscore: parseFloat(zscore), percentil: data };
  }
  let data = zscore2percentil.find((e) => e.zscore === parseFloat(zscore));
  return data;
};

const calculateZscore = (rawScore, mean, standardDeviation, method) => {
  let zscore, calculator, data;
  calculator = method ? inverseZscore : directZscore;
  zscore = calculator(rawScore, mean, standardDeviation);
  data = calculatePercentil(zscore);
  console.log(data);
  return { z: data.zscore, p: data.percentil };
};
